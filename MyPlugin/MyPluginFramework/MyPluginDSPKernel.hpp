...
#import "MyPluginParameterAddresses.h"

/*
 AudioPluginDSPKernel
 As a non-ObjC class, this is safe to use from render thread.
 */
class AudioPluginDSPKernel {
public:
    ...
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case MyPluginParameterAddress::gain:
                mGain = value;
                break;
                // Add a case for each parameter in MyPluginParameterAddresses.h
        }
    }
    ...
    void process(std::span<float const*> inputBuffers, std::span<float *> outputBuffers, AUEventSampleTime bufferStartTime, AUAudioFrameCount frameCount) {
        /*
         ...
         modify the check in [AudioPluginAudioUnit allocateRenderResourcesAndReturnError]
         */
